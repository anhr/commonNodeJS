...
*/		
				...
				if (!isNaN(i)) {
					...
					const angle2Vertice = () => {
						...
						if (classSettings.debug) {
							...
							if (Math.abs((sum - r)) > 4.5e-16) console.error(sHypersphere + ': Invalid vertice[' + i + '] sum = ' + sum);
					...
					case 'test': return () => {
						...
						_position.forEach((verticeAngles, verticeId) => {
							...
							for (let angleId = 0; angleId < verticeAngles.length; angleId++) {
								...
								const angle = verticeAngles[angleId], range = angles.ranges[angleId];
								if ((angle < range.min) || (angle > range.max)) console.error(sHypersphere + ': ' + range.angleName + ' angle[' + angleId + '] = ' + angle + ' of the vertice ' + verticeId + ' is out of range from ' + range.min + ' to ' + range.max);
							...
							addControllers: (fParent) => {
								...
								const vertices = [],
									pushVertice = (vertice) => {
										...
										else if (itemSize != vertice.length) console.error(sHypersphere + ': Middle vertice GUI. Invalid itemSize = ' + itemSize);
					...
					switch (classSettings.edges.creationMethod) {
						...
						case edgesCreationMethod.Random:
							...
							const progressBar = new ProgressBar(settings.options.renderer.domElement.parentElement, () => {
								...
								//else console.log(sHypersphere + '.pushEdges. Opposite vertice was not found.')
	...
	vertice2angles(vertice) {
		...
		//if (!this.classSettings.debug) console.warn(sHypersphere + ': Use vertice2angles in debug version only');
