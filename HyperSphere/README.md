# [HyperSphere](https://en.wikipedia.org/wiki/N-sphere)

If you click the ► button in the lower left corner of the canvas, you will start iterative process in which is resolving the [Thomson Problem](docs/ThomsonProblem.md),
at each step, all vertices gradually move toward a position in which the vertices are at the maximum distance from each other.

Examples:

[Circle](https://raw.githack.com/anhr/commonNodeJS/master/HyperSphere/Examples/circle.html).
[Sphere](https://raw.githack.com/anhr/commonNodeJS/master/HyperSphere/Examples/sphere.html).
[HyperSphere](https://raw.githack.com/anhr/commonNodeJS/master/HyperSphere/Examples/hyperSphere.html).

See [HyperSphere API](https://raw.githack.com/anhr/commonNodeJS/master/HyperSphere/jsdoc/index.html) and [RandomVertice API](https://raw.githack.com/anhr/commonNodeJS/master/HyperSphere/RandomVertice/jsdoc/index.html) for details.

 ## Have a job for me?
Please read [About Me](https://anhr.github.io/AboutMe/).
