...
class FrustumPoints
{
	 ...
	 * @param {FrustumPoints.cFrustumPointsF} [optionsShaderMaterial.cFrustumPoints] Adds a <b>FrustumPoints's</b> controllers into <a href="../../guiSelectPoint/jsdoc/index.html" target="_blank">Select Point</a> folder.
	 */
	constructor( THREE, camera, group, renderer, options, optionsShaderMaterial = {} ) {

		const cFrustumPoints = optionsShaderMaterial.cFrustumPoints;
		delete optionsShaderMaterial.cFrustumPoints;
//		this.optionsShaderMaterial = optionsShaderMaterial;
...
//		optionsShaderMaterial.stereo.lines = false;//сейчас lines не использую optionsShaderMaterial.stereo.lines === undefined ? true : optionsShaderMaterial.stereo.lines;//Display or hide lines between Frustum Points for more comfortable visualisation in the stereo mode.
		...
		const shaderMaterial = {},
			cookie = optionsShaderMaterial.cookie || new Cookie.defaultCookie(),
...
//		shaderMaterial.stereo.lines = optionsShaderMaterial.stereo.lines;
		...
		var points, names;
		const zeroPoint = new THREE.Vector3(), cameraDistanceDefault = camera.position.distanceTo( zeroPoint ), _this = this,
			lines = [], groupFrustumPoints = new THREE.Group();
//		this.lines = lines;
		var cloud = function () {
...
//			spatialMultiplex = spatialMultiplexNew;
			...
			const index = options.guiSelectPoint ? options.guiSelectPoint.getMeshIndex( points ) : undefined;

			update();

			//затем заменяю указатель на старый points в списке cMeshs на новый.
			if ( index ) options.guiSelectPoint.setIndexMesh( index, points );
...
/*			
			...
			guiParams.cookieName = 'FrustumPoints' + ( guiParams.cookieName ? guiParams.cookieName : '' );
*/			
			...
			*/
			...
			var folderPoint = new FolderPoint( fFrustumPoints, shaderMaterial.point, optionsShaderMaterial.point, function ( value ) {
...
/** @namespace
 * @description Adds a FrustumPoints's controllers into [dat.gui]{@link https://github.com/anhr/dat.gui}.
 */
FrustumPoints.cFrustumPointsF = function() {
