...
StereoEffect.getTextIntersection = function (intersection, options) {
					...
					intersection.object.geometry.attributes.ca === undefined || intersection.object.geometry.attributes.ca.itemSize < 4 ? '' : '\n' + lang$1.opacity + ': ' + new THREE.Vector4().fromArray(intersection.object.geometry.attributes.ca.array, intersection.index * intersection.object.geometry.attributes.ca.itemSize).w), intersection.pointSpriteText ? intersection.pointSpriteText : position, options.spriteOptions);
};
...
function ND(n) {
	...
	function create3DObject(geometry) {
		...
		if (!geometry.D3) {
			var nD = new ND(n, { object: { geometry: geometry } });
			...
			default:
				{
					...
					if (settings.indice === undefined) {
						for (var i = 0; i < segments.length; i++) {
							var nd = new ND(n, { object: { geometry: {
						...
						if (iSegments > 1) {
							for (var i = 0; i < segment.length; i++) {
								var _nd = new ND(n, { object: settings.object, indice: segment[i], iSegments: iSegments - 1 });
								if (n > 4) {
									if (n === 5) {
										var iIntersect = [];
	...
	function projectTo3D() {
		...
		object3D = create3DObject(geometry, { name: settings.object.name, color: 0x00ff00 });
	}
	projectTo3D();
		...
		object: {
			...
			set: function set$$1(object) {
				...
				if (object.geometry.indices) {
					...
					settings.object.geometry.indices = _indices2;
				}
				setIndices();
				setEdges();
