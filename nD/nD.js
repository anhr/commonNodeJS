...
/*
			const tetrahedron = new THREE.Mesh( new THREE.TetrahedronGeometry( ), new THREE.MeshLambertMaterial( {
					color: 0xffff00,
					opacity: 0.5,
					transparent: true
				} ) );
						...
						indices[1].forEach( face => {
							...
							face.forEach( ( edge, iEdge ) => {
								...
								if ( iEdge > 2 ) {
									...
									console.error( 'ND: PolyhedronGeometry: subdivide. invalid face edges count.');
									return;
								...
								function push( i ) {
									...
									if ( faceVertices.length > 2 ) {
										...
										console.error( 'ND: PolyhedronGeometry: subdivide. Invalid face vertices count = ' + faceVertices.length );
										return;
...
//									if ( !boPush ) console.error( 'ND: PolyhedronGeometry: subdivide. Missing push');
									if ( !boVertice0 && !boVertice1 ) console.error( 'ND: PolyhedronGeometry: subdivide. Missing push');
							...
							if ( faceVertices.length === 3 ) subdivideFace( faceVertices[0], faceVertices[1], faceVertices[2] );
...
*/
					...
					function subdivideFace( a, b, c, detail = 0 ) {
...
*/
/*				
				new THREE.Mesh( new THREE.TetrahedronGeometry( ), new THREE.MeshLambertMaterial( {
					color: 0xffff00,
					opacity: 0.5,
					transparent: true
				} ) ) :
*/
/*
				new THREE.Mesh( buffer.setIndex( indices ), new THREE.MeshLambertMaterial( {
					color: 0xffff00,
					opacity: 0.5,
					transparent: true,
...
*/
/*				
				new THREE.Mesh( buffer.setIndex( [
						0,1,2,
						0,1,3,
						0,2,3,
						1,2,3,
...
*/
/*				
				new THREE.Mesh( new THREE.BufferGeometry().setFromPoints( [
						new THREE.Vector3( 1, 1, 1),
						new THREE.Vector3( - 1, - 1, 1),
						new THREE.Vector3( - 1, 1, - 1),
//						new THREE.Vector3( 1, - 1, - 1),
					] ).setIndex( [2, 1, 0, 	0, 3, 2,	1, 3, 0,	2, 3, 1] ), new THREE.MeshLambertMaterial( {
					color: 0xffff00,
					opacity: 0.5,
					transparent: true,
					//side: THREE.DoubleSide
...
//object.material.side = THREE.BackSide; // back faces
//object.renderOrder = 0;
//object.geometry.index.array = [0,2,1]
			if ( settings3D.name )
