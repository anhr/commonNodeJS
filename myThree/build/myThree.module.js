...
var NumberControllerSlider$1 = function (_NumberController) {
	...
	function NumberControllerSlider(object, property, min, max, step, newBool) {
		...
		var _this2 = possibleConstructorReturn$1(this, (NumberControllerSlider.__proto__ || Object.getPrototypeOf(NumberControllerSlider)).call(this, object, property, {
			min: min,
			max: max,
			step: step
		}));
...
var ControllerFactory$1 = function ControllerFactory(object, property) {
	...
	if (Common$1.isNumber(initialValue)) {
		...
		if (Common$1.isNumber(arguments[4])) {
			return new NumberControllerBox$1(object, property, {
				min: arguments[2],
				max: arguments[3],
				step: arguments[4]
			});
...
var GUI$1 = function GUI(pars) {
	...
	if (!Common$1.isUndefined(params.load)) {
		...
		params.load = {
			preset: DEFAULT_DEFAULT_PRESET_NAME$1
		};
	}
...
function augmentController$1(gui, li, controller) {
	...
	if (controller instanceof NumberControllerSlider$1) {
		var box = new NumberControllerBox$1(controller.object, controller.property, {
...
class Three {
	...
	set THREE(THREE) {
		...
		*/
		...
		const
			//EventDispatcher = three.THREE.EventDispatcher,
		...
		this.create = function (elSliderWrapper, options) {
			...
			if (type == 'SVG') {
				try {
					...
					}, palette.getPalette())), CreateSVGElement('rect', {
						x: '0',
						y: '0',
						width: '100%',
						height: '100%',
						fill: 'url(#gradient-hsv-' + uniqID + ')'
					})]);
					if (slideIndicator !== undefined) {
...
function get(name, defaultValue) {
	...
	if (results) {
		...
		//boolean
		result === 'true' ? true : false : unescape(result); //string
	}
...
class defaultCookie {
	constructor(name) {
		...
		this.getObject = function (name, options, optionsDefault) {
			if (!optionsDefault) return; //object's settings is not saving
...
function getLanguageCode() {
	...
	function _getLocale() {
		...
		if (navigator.languages !== undefined && typeof navigator.languages !== 'unknown' //for IE6
		&& navigator.languages.length > 0) return navigator.languages[0]; //Chrome
...
SpriteText.getCenter = function (center = {}, position) {
	...
	return center instanceof THREE.Vector2 || typeof center === "object" && center.x !== undefined && center.y !== undefined //При копироваении и при чтении из cockie THREE.Vector2 превращается в Object{x: x, y: y}
	? center : screenPos ? new THREE.Vector2(screenPos.x < canvas.width / 2 ? 0 : 1, screenPos.y < canvas.height / 2 ? 1 : 0) : new THREE.Vector2(0, 1); //Default is left upper corner
};
function updateOptions(group, options) {
	if (group.userData.optionsSpriteText) Object.keys(group.userData.optionsSpriteText).forEach(function (key) {
		if (options[key] === undefined)
			//Child options have more priority before parent options
...
class StereoEffect {
	...
	constructor(renderer, options = new Options$1()) {
		...
		this.render = function (scene, camera) {
			...
			switch (spatialMultiplex) {
				...
				case spatialMultiplexsIndexs.TaB:
					...
					yL = 0;
					widthL = size.width;
					heightL = size.height / 2;
					xR = 0 - parallax;
					yR = size.height / 2;
					widthR = size.width;
					heightR = size.height / 2;
					setFullScreen(false, this);
					break;
				default:
		...
		this.gui = function (guiParams = {}) {
			...
			//Spatial multiplex
			const _controllerSpatialMultiplex = _fStereoEffects.add(settings, 'spatialMultiplex', _lang.spatialMultiplexs).onChange(function (value) {
			...
			const _controllerZeroParallax = _fStereoEffects.add(settings, 'zeroParallax', -_minMax, _minMax).onChange(function (value) {
		...
		this.createCanvasMenuItem = function (canvasMenu, params) {
			...
			params = params || {};
			const _lang = getLang({
					getLanguageCode: params.getLanguageCode,
					lang: params.lang
				}),
			...
			menuItemStereoEffect.select = function (value) {
				menuItemStereoEffect.items.forEach(function (item) {
					if (item.spatialMultiplex === value) {
						if (!item.checked) item.elName.onclick({
							target: item.elName
						});
					}
				});
...
function assign$1() {
	...
	Object.assign(THREE.Raycaster.prototype, {
		...
		setStereoEffect: function (settings = {}) {
			...
			const stereoEffect = settings.stereoEffect !== undefined ? settings.stereoEffect : typeof effect !== 'undefined' ? effect : new StereoEffect(renderer, settings.options),
				raycaster = this,
				mouseL = new THREE.Vector2(),
				mouseR = new THREE.Vector2();
			var particles,
				//The object or array of objects to check for intersection with the ray. See THREE.Raycaster.intersectObject https://threejs.org/docs/index.html#api/en/core/Raycaster.intersectObject for details.
			...
			function intersection(optionsIntersection) {
				if (mouse === undefined) return; //User has not moved mouse
				function isIntersection() {
					Options$1.raycaster.intersectionsInOut(particles, raycaster, renderer, mouse, settings);
				}
			...
			this.stereo = {
				...
				onDocumentMouseMove: function (event) {
					if (particles === undefined) return; //The object or array of objects to check for intersection with the ray is not defined. See THREE.Raycaster.intersectObject https://threejs.org/docs/index.html#api/en/core/Raycaster.intersectObject for details.
					event.preventDefault();
...
function createController(settings, controllerId, name, options) {
	...
	function setControllerValue(value) {
		if (settings.controller.value !== undefined) settings.controller.value = value; //input element type
...
class Options {
	...
	constructor(options) {
		const _this = this;
		options = options || {};
		if (options.boOptions) return options; //duplucate new Options
		...
		Object.defineProperties(this, {
			...
			dat: {
				get: function () {
					class Dat {
						...
						constructor(dat) {
							dat = dat || {};
							if (dat.boDat) return dat; //duplucate new Options
							function guiParent() {
					...
					options.dat.getCookieName = function (cookieName = '') {
						const name = options.dat.cookieName || (options.elContainer ? typeof options.elContainer === "object" ? options.elContainer.id : typeof options.elContainer === "string" ? options.elContainer : '' : '');
						...
						constructor(scales) {
							...
							Object.defineProperties(this, {
								...
								posAxesIntersection: {
									...
									set: function (posAxesIntersection) {
				...
				get: function () {
					...
					switch (typeof options.palette) {
						case 'number':
							...
							break;
						case 'boolean':
							if (options.palette) options.palette = new ColorPicker$1.palette(); // { palette: ColorPicker.paletteIndexes.BGYW } );
							break;
						case 'string':
						...
						default:
							{
								...
								if (Array.isArray(options.palette)) options.palette = new ColorPicker$1.palette({
									palette: options.palette
								}); //Custom palette
								else if (!options.palette.isPalette()) console.error('MyThree: invalid typeof options.palette: ' + typeof options.palette);
							}
					}
					return options.palette;
				}
			 ...
			 **/
			controllers: {
				//не засунул это в options.playerOptions потому что при чтение из cockie теряется HTMLElement
...
class Raycaster {
	...
	constructor() {
		...
		this.onMouseDown = function (intersection, options) {
			if (intersection.object.userData.isInfo !== undefined && !intersection.object.userData.isInfo()) return; //No display information about frustum point
			if (options.guiSelectPoint && intersection.object.userData.myObject) {
				...
				const guiPoints = intersection.object.userData.myObject.guiPoints,
			...
			constructor(camera, renderer, settings = {}) {
				...
				domElement.addEventListener('pointerdown', function (event) {
					...
					if (intersects && intersects.length > 0) {
						const intersect = intersects[0];
						if (intersect.object.userData.raycaster && intersect.object.userData.raycaster.onMouseDown) intersect.object.userData.raycaster.onMouseDown(intersect, event); //передаем состояние кнопок мыши
...
class Player$1 {
	...
	constructor(group, settings = {}) {
		...
		function onSelectScene(index) {
			...
			Player$1.selectPlayScene(group, {
				t: t,
				index: index,
				options: settings.options
			});
		...
		function step(timestamp) {
			...
			if (d > 0) while (timestamp > timeNext) timeNext += d;
			playNext();
		}
		...
		this.play3DObject = function () {
			...
			playing = true;
			if (options.playerOptions.max !== null && selectSceneIndex >= options.playerOptions.marks - 1) selectSceneIndex = 0; //-1;
			playNext();
			RenamePlayButtons();
		...
		*/
		this.localization = function (getLanguageCode = function () {
			return 'en';
		}) {
		...
		this.PlayController = class extends controllers.CustomController {
			...
			constructor(gui) {
				...
				const colorOff = 'rgb(255,255,255)',
					colorOn = 'rgb(128,128,128)';
				super({
					playRate: 1,
					//Default play rate is 1 changes per second
					property: function (customController) {
						var buttons = {};
						...
						if (player.getSettings().options.playerOptions.max !== null) {
							...
							function RenameRepeatButtons(isRepeat) {
								...
								if (buttons.buttonRepeat.title === title) return; //stop of infinite recursive call
			...
			set controller(newController) {
				...
				this._controller.onChange(function (value) {
					options.player.setTime(value); /*_this.onChange( value );*/
				});
		...
		this.gui = function (folder) {
			...
			Object.keys(options.playerOptions).forEach(key => {
				axesDefault[key] = options.playerOptions[key];
			});
			Object.freeze(axesDefault);
			const max = options.playerOptions.max,
				marks = options.playerOptions.marks; // interval = options.playerOptions.interval,
			cookie.getObject(cookieName, options.playerOptions, axesDefault);
			if (max === null || max === Infinity || options.playerOptions.max === null //раньше на веб странице плеер был настроен на бесконечное проигрыванияе а сейчас проигрывание ограничено по времени
			) {
			...
			function scale() {
				...
				scaleControllers.scaleController = scaleControllers.folder.add(new ScaleController(onclick, {
					settings: settings.options.playerOptions,
					getLanguageCode: getLanguageCode
				})).onChange(function (value) {
				...
				//Во время JSON преобразования Infinity превращается в null. Поэтому восстанавливаю обратно
		...
		* </pre>
		*/
		this.createCanvasMenuItem = function (canvasMenu, getLanguageCode = function () {
			return 'en';
		}) {
			_canvasMenu = canvasMenu;
			const player = this,
		...
		this.addSliderEvents = function () {
			...
			if (elSlider) {
				elSlider.onchange = function (event) {
					player.selectScene(parseInt(elSlider.value));
				};
				...
				var pointerdown;
				const player = this;
				elSlider.addEventListener('pointerdown', e => {
					pointerdown = true;
				});
				elSlider.addEventListener('pointerup', e => {
					pointerdown = false;
				});
...
Player$1.cameraTarget = class {
	...
	constructor() {
		...
		var boTarget = false,
			//true - target point was detected. For displaying of the console warning if duplicate target point was detected
		...
		this.init = function (cameraTarget, options, boErrorMessage = true) {
			...
			if (cameraTarget.bodefault !== false) {
				if (boTarget) return; //Не надо устанавливать cameraTargetDefault после того,
				...
				boTarget = true;
				if (boCameraTargetLook === undefined) cameraTargetDefault.boLook = true; //запрета на слежение камерой за точкой не было и есть точка, за которой надо следить
			} else return;
		...
		function setCameraTarget(cameraTarget) {
			assign();
			if (!cameraTarget) cameraTarget = cameraTargetDefault; //У выбранной для слежения точки нет cameraTarget
			if (!_cameraTarget.boMaual) {
				//если не делать эту проверку, то пользователь не сможет изменить вручную _cameraTarget.boLook когда выбрана точка, за которой надо следить
		...
		this.changeTarget = function (mesh, i, options) {
			...
			//Update cameraTarget
			const func = !mesh.userData.player || typeof mesh.userData.player.arrayFuncs === "function" ? {} : mesh.userData.player.arrayFuncs[mesh.userData.myObject.guiPoints.seletedIndex(i)];
			if (func != undefined) {
				if (!func.cameraTarget) func.cameraTarget = {
					boLook: false
				};
		...
		this.setCameraTarget = function (options) {
			...
			if (!camera) return; //В этом приложении невозможно следить за точкой, потому что ни разу не была вызывана Player.cameraTarget.init()
			...
			if (!cameraTarget.setCameraPosition) cameraTarget.setCameraPosition = function /*setCameraDefault*/
			() {
				var target = cameraTarget.target;

				//не менять позицию камеры если
				if (!cameraTarget.boLook ||
				//не следить за точкой
				//или
				!target &&
				//нет точки, за которой надо следить
				...
				if (options.orbitControls) {
					...
					if (options.orbitControls._listeners) options.orbitControls._listeners.change[0](); //move frustumpoints
				}
			};
...
Player$1.getPoints = function (arrayFuncs, optionsPoints) {
	...
	for (var i = 0; i < arrayFuncs.length; i++) {
		...
		if (Array.isArray(item)) arrayFuncs[i] = new THREE.Vector4(item[0] === undefined ? 0 : item[0], item[1] === undefined ? 0 : item[1], item[2] === undefined ? 0 : item[2], item[3] === undefined ? wDefault : item[3]);else if (typeof item === "object" && item instanceof THREE.Vector2 === false && item instanceof THREE.Vector3 === false && item instanceof THREE.Vector4 === false) {
	...
	for (var i = 0; i < arrayFuncs.length; i++) {
		...
		const point = funcs.vector instanceof THREE.Vector3 === true ? new THREE.Vector3(getAxis('x'), getAxis('y'), getAxis('z')) : new THREE.Vector4(getAxis('x'), getAxis('y'), getAxis('z'), getAxis('w'));
		if (funcs.cameraTarget) {
...
Player$1.getColors = function (arrayFuncs, optionsColor) {
	...
	for (var i = 0; i < length; i++) {
		...
		if (iColor >= optionsColor.colors.length) {
			...
			//w of the funcs is color of the point
			optionsColor.positions && optionsColor.positions.itemSize === 4 //w position of the positions is color of the point
				...
				if (funcs.w instanceof Object && funcs.w.func) {
					...
					min = optionsColor.options.scales.w.min;
					max = optionsColor.options.scales.w.max;
				}
			...
			} else if (optionsColor.colors instanceof THREE.Float32BufferAttribute) new THREE.Vector3(1, 1, 1);else if (optionsColor.color != undefined) {
...
Player$1.traceLine = class traceLine {
	...
	constructor(options) {
		...
		this.isVisible = function () {
			if (!options.player) return false; //не запущен Player(...)
		...
		this.addPoint = function (mesh, index, color) {
			...
			if (options.playerOptions.max === null) {
				...
				const point0 = arrayLines.length === 0 ? point : new THREE.Vector3().fromArray(arrayLines[arrayLines.length - 1].geometry.attributes.position.array, 1 * itemSize);
				point0.toArray(line.geometry.attributes.position.array, 0 * itemSize);
				line.geometry.attributes.position.needsUpdate = true;

				//point color
				if (color === undefined) color = new THREE.Color(1, 1, 1); //White
				Player$1.setColorAttribute(line.geometry.attributes, 0, arrayLines.length === 0 ? color : new THREE.Color().fromArray(arrayLines[arrayLines.length - 1].geometry.attributes.color.array, 1 * itemSize));
...
Player$1.selectPlayScene = function (group, settings = {}) {
	...
	if (typeof options.player === "object") options.player.endSelect = () => {
		Player$1.selectMeshPlayScene(group, {
			t: t,
			options: options
		});
		function selectMeshPlayScene(group) {
			group.children.forEach(function (mesh) {
				if (mesh instanceof THREE.Group) selectMeshPlayScene(mesh);else Player$1.selectMeshPlayScene(mesh, {
					t: t,
					options: options
				});
			});
...
class MyObject {
	...
	constructor(settings = {}, vertices) {
		...
		settings.overriddenProperties.getPlayerTimesLength = () => {
			return 1;
		};
		if (!settings.overriddenProperties.positionOffsetId) settings.overriddenProperties.positionOffsetId = positionId => {
			return positionId;
		};
		...
		const timeId = settings.options ? settings.options.player.getTimeId() : 0,
		...
		this.setVerticesRange = (start, count) => {
			const bufferGeometry = settings.bufferGeometry,
		...
		const createPositionAttribute = (pointLength, pointsLength) => {
			...
			const isRCount = settings.object.geometry.rCount != undefined,
				MAX_POINTS = isRCount ?
			...
			setDrawRangeTypes();
			if (MAX_POINTS != undefined) this.setVerticesRange(0, isRCount ? pointLength * pointsLength * getPlayerTimesLength() :
			//зарезервировано место для вершин вселенной с разным радиусом
			...
			if (isRCount) settings.bufferGeometry.userData.drawRange = () => {
			...
			settings.bufferGeometry.userData.positionOffsetId = positionId => {
				return this.positionOffsetId(positionId);
			};
									...
									case 'vector':
										...
										}
										{
											...
											switch (vertice.length) {
												case 3:
													return new THREE.Vector3(vertice[0], vertice[1], vertice[2]);
												case 4:
													return new THREE.Vector4(vertice[0], vertice[1], vertice[2], vertice[3]);
											}
			...
			const itemSize = settings.object.geometry.opacity ? 4 : 3,
		...
		this.setPositionAttributeFromPoints = (points, boCreatePositionAttribute) => {
			const bufferAttributes = settings.bufferAttributes,
			...
			if (!bufferGeometry.attributes.position) {
				createPositionAttribute(this.pointLength ? this.pointLength() : points[0].w === undefined ? 3 : 4, points.length);
		...
		this.setPositionAttributeFromPoint = (i, vertice, timeId) => {
			...
			let itemSize = positionData.itemSize,
			...
			} else if (Array.isArray(verticeColor)) verticeColor.forEach(item => array[colorId++] = item);else if (verticeColor instanceof THREE.Color) {
		...
		this.color = () => {
			...
			return color != undefined ? typeof color === "function" ? color() : color : this.defaultColor; //'lime';
		};
		/**
	...
	*/
	constructor(group, arrayFuncs, onReady, settings = {}) {
		...
		if (arrayFuncs instanceof THREE.BufferGeometry) {
			geometry = arrayFuncs;
			arrayFuncs = [];
			for (var i = 0; i < geometry.attributes.position.count; i++) arrayFuncs.push(new THREE.Vector3().fromBufferAttribute(geometry.attributes.position, i));
			...
			if (settings.pointsOptions.bufferGeometry) geometry = settings.pointsOptions.bufferGeometry;else {
				const points = arrayFuncs != undefined ? Player$2.getPoints(arrayFuncs, {
					options: settings.options,
					group: group,
					t: tMin
				}) : undefined;
		...
		*/
		...
		const texture = new THREE.TextureLoader().load(currentScriptPath$1 + "/textures/point.png", function (texture) {}, function () {}, function () {
		...
		loadShaderText(function (shaderText) {
			...
			if (settings.pointsOptions && settings.pointsOptions.frustumPoints) points.userData.cloud = {
				indexArray: indexArrayCloud
			};
			points.userData.shaderMaterial = settings.pointsOptions === undefined ? settings.shaderMaterial : settings.pointsOptions.shaderMaterial;
			onReady(points);
...
class MyPoints extends MyObject {
	...
	constructor(arrayFuncs, group, settings) {
		...
		//вершины заданы в settings.bufferGeometry
		...
		if (pointsOptions.name !== '' && pointsOptions.elements) {
			...
			const elPointsName = typeof pointsOptions.elements.pointsName === "string" ? document.getElementById(pointsOptions.elements.pointsName) : pointsOptions.elements.pointsName;
		...
		if (pointsOptions.shaderMaterial !== false) new getShaderMaterialPoints(group, arrayFuncs, function (points) {
			...
			if (typeof arrayFuncs === 'function') buffer = arrayFuncs();else {
				const points = Player$2.getPoints(arrayFuncs, {
					options: options,
					group: group,
					t: pointsOptions.tMin
				});
				this.getPoint = i => {
					return points[i];
				};
		...
		function Points(points) {
			...
			if (pointsOptions.controllers !== undefined) {
				...
				points.userData.controllers = function /*cFrustumPoints*/ () {
					return pointsOptions.controllers(/*cFrustumPoints*/);
				};
			}
			...
			if (arrayFuncs instanceof THREE.BufferGeometry) {
				arrayFuncsPlayer = [];
				for (var i = 0; i < arrayFuncs.attributes.position.count; i++) arrayFuncsPlayer.push(new THREE.Vector3().fromBufferAttribute(arrayFuncs.attributes.position, i));
		...
		if (menuItem.items) {
			...
			menuItem.items.forEach(function (itemItem) {
				...
				function getItemName(item) {
					const str = typeof item === 'string' ? item : item.radio === true ? (item.checked ? '◉' : '◎') + ' ' + item.name : item.checkbox === true ? (item.checked ? '☑' : '☐') + ' ' + item.name : item.name; //✓🗹
					//console.log(str);
					return str;
				}
				...
				if (typeof itemItem === 'string') ;else {
					itemItem.name;
...
class CanvasMenu {
	...
	constructor(renderer, settings = {}) {
		...
		if (settings.options.stereoEffect && settings.options.stereoEffect.createCanvasMenuItem) settings.options.stereoEffect.createCanvasMenuItem(this, {
			getLanguageCode: options.getLanguageCode
		});
		if (options.player) {
		...
		this.setSize = function (width) {
			...
			if (!elSlider) return; //no controllerPlay
...
function clearThree$1(obj) {
	...
	if (obj.geometry) {
		if (obj.geometry.attributes !== undefined) Object.keys(obj.geometry.attributes).forEach(prop => {
	...
	if (obj.material) {
		...
		if (obj.material.uniforms !== undefined) obj.material.uniforms.pointTexture.value.dispose(); //Texture
	...
	constructor(group, options) {
		...
		*/
		...
		this.createAxis = function (axisName) {
			...
			new THREE.Vector3(
			//X
			axisName !== 'x' ? 0 : !options.scales.x ? 0 //X axis is not exists
			: options.scales.x.min,
			//begin of the X axix
			//Y
			axisName !== 'y' ? 0 : !options.scales.y ? 0 //Y axis is not exists
			: options.scales.y.min,
			//begin of the Y axix
			//Z
			axisName !== 'z' ? 0 : !options.scales.z ? 0 //Z axis is not exists
			...
			if (scale.marks !== undefined) {
				const SpriteMark = function (position) {
					...
					function update() {
						...
						//x
						axisName !== 'y' ? 0.5 :
						//For x and z axes риска не сдвигается
						0,
						//For y axes риска сдвигается вправо

						//y
						axisName === 'y' ? 0.5 :
						//For y axes риска не сдвигается
						...
						function getCenterX() {
							const a = (0.013 - 0.05) / 15,
								b = 0.013 - 17 * a;
							return -width * (getTextPrecision().length * a + b);
						}
						const spriteText = new SpriteText(getTextPrecision(), new THREE.Vector3(position.x, position.y, position.z), {
							group: group,
							rotation: axisName === 'y' ? 0 : -Math.PI / 2,
							center: new THREE.Vector2(getCenterX(),
							//текст по оси y сдвигается вправо
							//текст по оси x и z сдвигается вниз

							axisName === 'x' ? 1 :
							//текст по оси x сдвигается влево
			...
			this.dottedLines = function (_intersection) {
				...
				if (groupDotLines !== undefined) {
					function dottedLine(axisName) {
						...
						if (!line) return; //Current axis is not exists
...
function SpriteTextGui(group, options, guiParams = {}) {
	...
	function updateSpriteText(noSave) {
		...
		SpriteText.updateSpriteTextGroup(group);
		if (group.userData.update) group.userData.update(); // optionsSpriteText );
	...
	if (optionsSpriteText.hasOwnProperty(textHeight) && optionsSpriteText[textHeight] !== undefined) {
		ScaleControllers(fSpriteText, optionsSpriteText, textHeight, function () {
			...
			text: lang.textHeight,
			textTitle: lang.textHeightTitle,
			...
			function setValues(folder, key, optionsDefault) {
				folder.__controllers.forEach(function (controller) {
					if (controller.property !== key) {
						if (typeof optionsDefault[key] !== "object") return;
	...
	const defaultParams = {
		defaultF: fSpriteText.userData.restore
	};
	...
	fAxesHelper.add(new ScaleController(function (customController, action) {
		function zoom(zoom, action) {
			function axesZoom(axes, scaleControllers) {
				if (axes === undefined) return; //not 3D axesHelper
...
class functionsFolder {
	...
	*/
	constructor(fParent, onFinishChange, options, vector) {
		...
		const _this = this;
		var boError = false,
			//true - обнаружена ошибка ввода. Нужно вывести сообщение об ошибке и вернуть фокус на поле управления
		...
		function getFuncText(func) {
			...
			switch (typeofFunc) {
				case 'number':
					func = func.toString();
				//если это не делать будет создан NumberControllerBox, ктороый не позволяет вводить float
				case 'string':
					return func;
				case 'function':
...
class GuiSelectPoint {
	...
	constructor(options, guiParams = {}) {
		...
		var f3DObjects,
			...
			funcFolder,
			boSetMesh = false,
			//Для предотвращения лишних вызовов exposePosition если выбрать точку и передвинуть камеру с помошью OrbitControls,
			fRotation,
		...
		function displayPointControllers(display) {
			...
			if (cCameraTarget) {
				...
				//Камера не может следить за frustumPoints
				display;
			}
		}
		...
		function exposePosition(selectedPointIndex) {
			if (selectedPointIndex === undefined) selectedPointIndex = guiSelectPoint.getSelectedPointIndex(); //Эта строка слишком медленно выполняется если число точек frustumPoints велико
			...
			if (options.axesHelper !== undefined)
				...
				if (options.axesHelper !== false && options.axesHelper !== undefined) options.axesHelper.exposePosition({
					object: mesh,
					index: selectedPointIndex
				});
		...
		function setPosition(intersectionSelected) {
			...
			*/
			...
			const func = player && player.arrayFuncs ? player.arrayFuncs.intersection ? player.arrayFuncs.intersection(intersectionSelected.index) : player.arrayFuncs[intersectionSelected.index] : undefined,
			...
			if (!attributes.color) {
				...
				if (attributes.position.itemSize < 4) {
					...
					function isWObject() {
						return typeof func.w === 'object' && func.w instanceof THREE.Color === false;
					}
					const mesh = getMesh(),
						verticeColor = mesh.userData.myObject ? mesh.userData.myObject.verticeColor(intersectionSelectedIndex) : undefined;
					var color = func === undefined || !attributes.color && !attributes.ca ? undefined : Array.isArray(func.w) || typeof func.w === "function" ? Player$2.execFunc(func, 'w', options.time, options) : isWObject() ? Player$2.execFunc(func.w, 'func', options.time, options) : typeof func.w === "string" ? Player$2.execFunc(func, 'w', options.time, options) : verticeColor != undefined ? verticeColor : func.w;
		...
		this.addMesh = function (mesh) {
			...
			for (var i = 0; i < cMeshs.__select.options.length; i++) {
				...
				if (mesh.userData.boFrustumPoints && option.mesh !== undefined && option.mesh.userData.boFrustumPoints) return; //duplicate FrustumPoints. Сюда попадает когда пользователь меняет количество слоев или Y точек в FrustumPoints.
		...
		this.select = function (intersectionSelected) {
			...
			if (!myObject.getPositionData) myObject.getPositionData = index => {
				return {
					verticeId: index,
					positionId: index * intersectionSelected.object.geometry.attributes.position.itemSize
				};
			};
			...
			if (!index) return; //Сюда попадает когда Mesh проверяется с помощью THREE.Raycaster находится ли он под мышью, но этот Mesh не внесен в список GuiSelectPoint
			if (cMeshs.__select[index].selected === false) {
				...
				if (cCustom) {
					...
					//Пользователь нажал не левую кнопку мыши. Надо восстановить выбранный nD объект
					dat, options);
				}
			}
			this.selectPoint2 = function (selectedMesh) {
				...
				if (selectedMesh !== undefined && !Object.is(intersectionSelected.object, selectedMesh)) return; //Сначала пользователь выбрал точку с помошщью мыши
				...
				const mesh = getMesh(),
					...
					//You can not trace points if you do not defined the mesh.userData.player.arrayFuncs
					undefined;
		...
		this.getSelectedPointIndex = function () {
			...
			//FrustumPoints folder is visible
			options.frustumPoints && options.frustumPoints.isDisplay() //The cDisplay checkbox of the frustumPoints' is checked
			) {
		...
		function visibleTraceLine(intersection, value, getMesh) {
			...
			if (color === undefined) color = new THREE.Color(0xffffff); //white
			else {
		...
		function displayVerticeID(object) {
			...
			for (let i = drawRange.start; i < count; i++) {
				...
				const spriteText = new SpriteText(i, p, {
					group: object
				});
		...
		function addPoints(mesh, intersectionSelected) {
			...
			const count = mesh.geometry.userData.drawRange ? mesh.geometry.userData.drawRange().count :
			//Во вселенной задал диапазон видимых вершин
			...
			//диапазон вершин больше количества вершин
		...
		this.add = function (folder) {
			folder = folder || options.dat.gui;
			if (!folder) return; //gui не создан
			...
			cMeshs = f3DObjects.add({
				Meshs: lang.notSelected
			}, 'Meshs', {
				[lang.notSelected]: -1
			}).onChange(function (value, intersectionSelected) {
				value = parseInt(value);
			...
			cPoints.onChange(function (pointId) {
				...
				if (isNaN(pointId)) {
					...
					if (onchange) onchange();else console.error('GuiSelectPoint: cPoints.onChange. Under constraction'); //во вселенной выбрать все точки для текущего времени проигрывателя
					return;
				}
				var display;
				let mesh = getMesh();
				...
				if (pointId === -1) display = 'none';else {
					display = 'block';
					const userData = mesh.userData.myObject.bufferGeometry.userData,
				...
				if (options.axesHelper !== false && options.axesHelper !== undefined) options.axesHelper.exposePosition(getObjectPosition(mesh, pointId //((pointId != -1) ? mesh.userData.myObject.guiPoints.positionOffset : 0) + pointId
				));
				...
				if (playerCameraTarget) {
					cCameraTarget = fPoints.add(playerCameraTarget, 'boLook').onChange(function (boLook) {
						...
						for (var i = 0; i < cMeshs.__select.options.length; i++) {
							...
							if (!mesh || !mesh.userData.player || !mesh.userData.player.arrayFuncs) continue;
							const arrayFuncs = mesh.userData.player.arrayFuncs;
							for (var j = 0; j < arrayFuncs.length; j++) if (arrayFuncs[j].cameraTarget) arrayFuncs[j].cameraTarget.boLook = false;
						}
						...
						if (orbitControlsOptions) {
							...
							if (Player$2.orbitControls) Player$2.orbitControls.target.copy(orbitControlsOptions.target);
			...
			if (!options.traces) options.traces = {
				...
				onChange: value => {
					...
					for (var i = 0; i < mesh.geometry.attributes.position.count; i++) visibleTraceLine({
						object: mesh,
						index: i
					}, value, getMesh);
		...
		function addPointControllers() {
			...
			function axesGui(axisName) {
				...
				if (axisName === 'w') {
					...
					if (scale.isAxis()) controller = fPoint.add({
						value: scale.min
					}, 'value', scale.min, scale.max).onChange(function (value) {
			...
			cTrace = fPoint.add({
				boTrace: false
			}, 'boTrace').onChange(function (value) {
			...
			function axesWorldGui(axisName) {
				const scale = options.axesHelper === undefined || options.axesHelper === false ? options.scales[axisName] : options.axesHelper.options ? options.axesHelper.options.scales[axisName] : undefined;
				if (!scale.isAxis()) return;
				const controller = dat.controllerZeroStep(fPointWorld, {
					value: scale.min
				}, 'value');
				...
				defaultF: function () {
					const positionDefault = intersection.object.userData.player.arrayFuncs[intersection.index],
						...
						setDefaultValue = (control, value) => {
							if (!control) return;
							control.setValue(typeof value === "function" ? value(t, options.a, options.b) : typeof value === "string" ? Player$2.execFunc({
								w: value
							}, 'w', options.time, options) : value);
						};
					setDefaultValue(cX, positionDefault.x);
					setDefaultValue(cY, positionDefault.y);
					setDefaultValue(cZ, positionDefault.z === undefined ? 0 :
					//default Z axis of 2D point is 0
					positionDefault.z);
...
class MoveGroupGui {
	...
	constructor(group, options, guiParams = {}) {
		...
		if (options.axesHelper && options.axesHelper.options) options.scales = options.axesHelper.options.scales;else options.scales = options.scales || {
			x: {},
			y: {},
			z: {}
		};
		...
		function setDefault(axisName) {
			...
			if (!scale) return;
			options.moveGroupGui = options.moveGroupGui || {
				scales: {}
			};
	...
	constructor(camera, options, gui) {
		...
		if (options.player) {
			...
			controllerLook = fCamera.add(cameraTarget, 'boLook').onChange(function (boLook) {
				...
				if (boLook) return;
				if (options.orbitControls) options.orbitControls.reset(); //обязательно вызвать controls.saveState();
			});
		...
		const defaultPosition = {
			...
			defaultTarget = options.orbitControls ? {
				...
				y: 0,
				z: 0
			}; //default camera look at zero coordinate
...
class FrustumPoints {
	...
	constructor(camera, group, canvas, settings = {}) {
		...
		this.pushArrayCloud = function (geometry) {
			...
			if (points) points.userData.cloud = {
				indexArray: index
			};
			return index;
		};
		...
		this.create = function (renderer) {
			...
			//
			if (_arrayCloud.length === 0) return undefined; //нет точек с облаком. Поэтому нет смысла создавать frustumPoints
			...
			var cloud = function () {
				...
				this.create = function (_uniforms) {
					...
					const max = options.scales.w.max,
						min = options.scales.w.min;
					...
					distanceTableWidth, 'distanceTable', {
						...
						onReady: function (data, itemSize, updateItem) {
							...
							var dx = 0.5 / (distanceTableWidth - 1);
							const ddx = 1.001;
				...
				this.addUniforms = function (format, width, key, optionsAddUniforms) {
					...
					const height = optionsAddUniforms.height || 1,
						//format === THREE.LuminanceFormat ? 1 : 2,
						size = width * height,
				...
				this.updateMesh = function (mesh) {
					if (mesh.userData.cloud === undefined || !this.cloudPoints //Пользователь убрал галочку в 'Display' - 'Display or hide Frustum Points.'
					) return;
					for (var i = 0; i < mesh.geometry.attributes.position.count; i++) {
			...
			function progress(onReady) {
				...
				function Z(z) {
					...
					const ynStep = (point_n3.y - point_n1.y) / (yCount - 1),
						...
						//координату точки надо немного сдвинуть в зависимости от z что бы точки не накладывались друг на друга
					...
					for (var y = 0; y < yCount; y++) {
						for (var x = 0; x < xCount; x++) if (z >= zStart) {
				...
				function eachZ(zStart, zEnd) {
					...
					if (zMid === zStart) return; //for testing set 'Z Count' = 6
					Z(zMid);
				...
				_this.pointIndexes = function (pointIndex) {
					...
					function getObject() {
						index--;
						while (index >= 0 && typeof _names[index] !== "object") index--;
						name = _names[index];
					}
					function getZ() {
						while (index >= 0 && name.z === undefined) getObject();
					}
				...
				new MyPoints(function () {
					...
					pointsOptions: {
						...
						opacity: true,
						pointIndexes: function (pointIndex) {
						...
						controllers: function () {
							if (_guiSelectPoint) _guiSelectPoint.appendChild({
								xCount: xCount,
								yCount: yCount,
								zCount: zCount
							});
						...
						uniforms: function (uniforms) {
							...
							uniforms.cameraPositionDefault = {
								value: cameraPositionDefault
							};
							uniforms.cameraQuaternion = {
								value: camera.quaternion
							};
							...
							new cloud.addUniforms(THREE.RGBAFormat, 256, 'palette', {
								onReady: function (data, itemSize, updateItem) {
									options.scales.setW();
									const min = options.scales.w.min,
										max = options.scales.w.max;
									const size = data.length / itemSize;
									for (var i = 0; i < size; i++) updateItem(i, options.palette.toColor((max - min) * i / (size - 1) + min, min, max));
								}
							});
							return cloud;
						},
						onReady: function (points) {
							...
							if (onReady !== undefined) onReady(); //Пользователь изменил настройки frustumPoints
			...
			this.update = function (onReady) {
				...
				setPointsParams();
				var cameraQuaternionDefault = new THREE.Vector4(camera.quaternion.x, camera.quaternion.y, camera.quaternion.z, camera.quaternion.w);
				if (!_points) return; //User has changed 'Z Count' of the frustumPoints
			...
			function removePoints(notRemoveMesh) {
				...
				if (!notRemoveMesh && options.guiSelectPoint) options.guiSelectPoint.removeMesh(_points); //не удаляю frustumPoints из списка Meshes потому что сюда попадает только если пользователь изменил число точек frustumPoints.
			...
			*/
			...
			this.gui = function (folder) {
				...
				const cNear = fFrustumPoints.add(shaderMaterial, 'near', 0, 100, 1).onChange(function (value) {
					update();
				});
				dat.controllerNameAndTitle(cNear, lang.near, lang.nearTitle);

				//Shift of the frustum layer far to the camera in percents
				const cFar = fFrustumPoints.add(shaderMaterial, 'far', 0, 100, 1).onChange(function (value) {
					update();
				});
				dat.controllerNameAndTitle(cFar, lang.far, lang.farTitle);

				//Scale of the base of the frustum points in percents.
				const cBase = fFrustumPoints.add(shaderMaterial, 'base', 0, 100, 1).onChange(function (value) {
					update();
				});
				dat.controllerNameAndTitle(cBase, lang.base, lang.baseTitle);

				//Square base of the frustum points.
				const cSquare = fFrustumPoints.add(shaderMaterial, 'square').onChange(function (value) {
					update();
				});
				...
				var toUpdate = true,
					//Когда пользователь нажимает кнопку Default надо установить toUpdate = false
				...
				//zCount
				const cZCount = fFrustumPoints.add(shaderMaterial, 'zCount').min(3).step(1).onChange(function (value) {
					update();
				});
				dat.controllerNameAndTitle(cZCount, lang.zCount, lang.zCountTitle);

				//yCount
				const cYCount = fFrustumPoints.add(shaderMaterial, 'yCount').min(3).step(1).onChange(function (value) {
					update();
				});
		...
		this.guiSelectPoint = function () {
			...
			this.appendChild = function (count) {
				function appendChild(cFrustumPoint, count) {
					//thanks to https://stackoverflow.com/a/48780352/5175935
					cFrustumPoint.domElement.querySelectorAll('select option').forEach(option => {
			...
			this.pointIndexes = function (index) {
				if (index === undefined) return; //Сюда попадает в отладочной версии когда не заданы имена точек
			...
			this.getSelectedIndex = function () {
				...
				for (var i = 0; i < _names.length; i++) {
					var name = _names[i];
					if (typeof name !== "object" || name.z === undefined || name.z !== z) continue;
					for (; i < _names.length; i++) {
						name = _names[i];
						if (typeof name !== "object" || name.y !== y) continue;
						for (; i < _names.length; i++) {
							name = _names[i];
							if (typeof name === "object") {
								if (x === 0 && name.y === y) {
...
class pointLight {
	...
	constructor(scene, settings = {}) {
		...
		this.controls = function (guiParams = {}) {
			const group = guiParams.group || scene,
			...
			//displayLight
			dat.controllerNameAndTitle(fLight.add({
				display: false
			}, 'display').onChange(function (value) {
				if (value) {
					new MyPoints(light.position, group, {
						pointsOptions: {
							onReady(points) {
								lightSource = points;
							}
	...
	constructor(createXDobjects, options) {
		...
		*/
		...
		const defaultPoint = {};
			...
			new THREE.Vector2();
		var renderer, fOptions, rendererSizeDefault, cameraPosition, pointSize, stats,
		...
		function init() {
			...
			options.eventListeners = new Options$1.raycaster.EventListeners(camera, renderer, {
				options: options,
				scene: scene
			});
			...
			if (options.dat.gui) {
				...
				if (options.dat.gui.__closeButton.click !== undefined)
					//for compatibility with Safari 5.1.7 for Windows
			...
			if (!options.player) {
				Player$2.selectPlayScene(group, {
					options: options
				}); //, Player.getTime(), 0 );
			}
			...
			if (options.dat.gui) {
				...
				pointLight2.controls({
					group: group,
					folder: fOptions,
					folderName: lang.light + ' 2'
				});
				...
				const folderPoint = new FolderPoint(options.point, function (value) {
					...
					group.children.forEach(function (mesh) {
						...
						if (mesh.material.uniforms === undefined) mesh.material.size = value / options.point.sizePointsMaterial; //PointsMaterial
...
MyThree.limitAngles = function (rotation) {
	function limitAngle(axisName) {
		while (rotation[axisName] > Math.PI * 2) rotation[axisName] -= Math.PI * 2;
	}
	limitAngle('x');
	limitAngle('y');
	limitAngle('z');
