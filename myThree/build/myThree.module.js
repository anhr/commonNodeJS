...
StereoEffect.getTextIntersection = function (intersection, options) {
					...
					intersection.object.geometry.attributes.ca === undefined || intersection.object.geometry.attributes.ca.itemSize < 4 ? '' : '\n' + lang.opacity + ': ' + new THREE.Vector4().fromArray(intersection.object.geometry.attributes.ca.array, intersection.index * intersection.object.geometry.attributes.ca.itemSize).w), intersection.pointSpriteText,
...
function Raycaster() {
			...
			this.onIntersection = function (intersection, options, scene, camera, renderer) {
						...
						} else spriteTextIntersection.position.copy(intersection.pointSpriteText);
			};
			...
			this.intersectionsInOut = function (particles, raycaster, renderer, mouse) {
						...
						intersects.forEach(function (intersection) {
									...
									if (settings.options.stereoEffect.settings.spatialMultiplex === StereoEffect.spatialMultiplexsIndexs.Mono) raycaster.ray.at(three.options.camera.near + (three.options.camera.far - three.options.camera.near) / 1000, intersection.pointSpriteText);
...
function MyPoints(arrayFuncs, group, settings) {
	...
	function (points) {
		...
		var points = new THREE.Points(typeof arrayFuncs === 'function' ? arrayFuncs() : new THREE.BufferGeometry().setFromPoints(Player$1.getPoints(arrayFuncs, { options: options, group: group, t: pointsOptions.tMin }), 4), new THREE.PointsMaterial({ size: options.point.size / options.point.sizePointsMaterial, vertexColors: THREE.VertexColors }));
