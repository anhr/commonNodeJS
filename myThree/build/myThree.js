...
function getShaderMaterialPoints(group, arrayFuncs, onReady, settings) {
	...
	loadShaderText(function (shaderText) {
		...
		Player$1.selectMeshPlayScene(points, { options: settings.options });
...
function MyPoints(arrayFuncs, group, settings) {
	...
	function (points) {
		...
		var points = new THREE.Points(typeof arrayFuncs === 'function' ? arrayFuncs() :
		new THREE.BufferGeometry().setFromPoints(Player$1.getPoints(arrayFuncs, { options: options, group: group, t: pointsOptions.tMin }), 4), new THREE.PointsMaterial({ size: options.point.size / options.point.sizePointsMaterial, vertexColors: THREE.VertexColors }));
	...
	function Points(points) {
		...
		points.userData.raycaster = {
			onIntersection: function onIntersection(intersection, mouse) {
				if (options.raycaster && options.raycaster.onIntersection) options.raycaster.onIntersection(intersection, mouse);
			},
			...
			onMouseDown: function onMouseDown(intersection) {
				...
				if (options.raycaster.onMouseDown) options.raycaster.onMouseDown(intersection);
			}
		};
...
function StereoEffect(renderer) {
					...
					this.getRendererSize = function () {
										var el = settings.elParent || renderer.domElement,
										    style = {
		...
		if (scale.marks !== undefined) {
			var SpriteMark = function SpriteMark(position) {
				...
				function update() {
					...
					var spriteText = new SpriteText(getTextPrecision(), new THREE.Vector3(position.x, position.y, position.z), {
						group: group,
...
function SpriteTextGui(group, options) {
	...
	function updateSpriteText(noSave) {
		...
		if (!noSave) cookie$$1.setObject(cookieName, optionsSpriteText);
	}
...
function MoveGroupGui(group, options) {
			...
			var cookie$$1 = options.cookie || new cookie.defaultCookie(),
			    dat = three$1.dat,
			...
			cookie$$1.getObject(cookieName, optionsGroup, groupOptionsDefault);
			...
			function setSettings() {
						...
						cookie$$1.setObject(cookieName, optionsGroup);
			}
			function scale(axes,
				...
				function onloadScripts() {
								...
								function init() {
												...
												options.raycaster.addParticle = function (item) {
																if (raycaster.stereo !== undefined) raycaster.stereo.addParticle(item);
												};
												options.raycaster.removeParticle = function (item) {
																if (raycaster.stereo !== undefined) raycaster.stereo.removeParticle(item);
												};
												if (options.dat.gui) {
												...
												group.children.forEach(function (item) {
																if (item.userData.raycaster !== undefined) {
																				if (raycaster.stereo !== undefined) {
																								if (!raycaster.stereo.isAddedToParticles(item))
								...
								function onObjectMouseDown(position, intersection) {
												if (options.axesHelper !== undefined && intersection.object.type === "Points") options.axesHelper.exposePosition(position);else alert('You are clicked the "' + intersection.object.type + '" type object.' + (intersection.index === undefined ? '' : ' Index = ' + intersection.index + '.') + ' Position( x: ' + position.x + ', y: ' + position.y + ', z: ' + position.z + ' )');
								}
